...
// isResetPassword: 이메일을 찾은 후 비밀번호 초기화 요청 여부
// find-email과 reset-password가 형식이 같아서 합쳤습니다
type FindEmailFormProps = {
  isResetPassword: boolean;
};

const FindEmailForm = ({ isResetPassword }: FindEmailFormProps) => {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Button>{isResetPassword ? '비밀번호 찾기' : '이메일 찾기'}</Button>
      </form>
    </Form>
  );
};
